...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.E = nn.Sequential(
            # TODO: setting1
            nn.Conv2d(3 + 15, 32, kernel_size=3, padding=1),
            # TODO: setting2
            # nn.Conv2d(3 + 18, 32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
    ...
    def forward(self, x):
        fea = self.E(x)
        out = self.Avg(fea).squeeze(-1).squeeze(-1)
        out = self.mlp(out)
...
class BlindSR(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            x, map = x
            encoder_input = torch.cat((x, map), dim=2)

            fea, contrast_loss = self.E(encoder_input)
            sr, _, _ = self.G(x[:, 0, ...], fea)

            return sr, contrast_loss
        else:
            x, map = x
            fea = self.E(torch.cat((x, map), dim=1))
