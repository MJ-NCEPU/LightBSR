    ...
    def __call__(self, hr_tensor, random=True):
        with torch.no_grad():
            ...
            if self.noise > 0:
                ...
                if random:
                    noise_level = torch.rand(B, 1, 1, 1, 1).to(lr_blured.device) * self.noise
                else:
                    noise_level = (torch.ones(B, 1, 1, 1, 1, dtype=torch.float32) * self.noise).to(lr_blured.device)
                noise = torch.randn_like(lr_blured).view(-1, N, C, H_lr, W_lr).mul_(noise_level).view(-1, C, H_lr, W_lr)
                lr_blured.add_(noise)
            else:
                noise_level = torch.rand(B, 1, 1, 1, 1).to(lr_blured.device) * 0.0

            lr_blured = torch.clamp(lr_blured.round(), 0, 255) / 255.

            return lr_blured.view(B, N, C, H // int(self.scale), W // int(self.scale)), b_kernels, noise_level
